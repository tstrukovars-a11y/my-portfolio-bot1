...
# Вес стадии: чем ближе к титулу, тем интереснее исход. Нужен там, где
# из десятка матчей надо выбрать один — например, под какой спрашивать
# прогноз. Первый круг «Шлема» и финал турнира — события разного веса,
# а по списку они идут вперемешку.
_STAGE = {
    "финал": 100,
    "1/2 финала": 90,
    "1/4 финала": 80,
    "1/8 финала": 70,
    # Групповой этап бывает только на Итоговом, где все восемь — из топа.
    "групповой этап": 65,
    "1/16 финала": 60,
    "1/32 финала": 50,
    "1/64 финала": 40,
...
def stage(name: str) -> int:
    """Насколько поздняя стадия. Неизвестная — ноль, а не догадка."""
    return _STAGE.get(rnd(name), 0)
