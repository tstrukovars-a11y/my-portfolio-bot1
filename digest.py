...
SCHEDULE = [
    ("08:00", "morning",  "Доброе утро. Вот что случилось вчера."),
    ("08:40", "results",  None),
    ("09:30", "tennis",   None),
    ("11:00", "books",    None),
    ("13:00", "genetics", None),
...
# На выходных первый выпуск на час позже. В субботу человек не встаёт по
# будильнику, и восемь утра для него — ночь: пост уходит в непрочитанное и
# теряется под тем, что вышло позже. Сдвигаем только утро: остальная сетка
# держит день, и двигать её незачем.
# Двигать одно утро было нельзя: итоги матчей в 08:40 оказались бы раньше
# него и стали бы первым, что человек видит в субботу. Поэтому на час
# уезжает весь утренний блок целиком, порядок внутри дня сохраняется.
WEEKEND_LATER = {"morning": 60, "results": 60, "tennis": 60}


def _slot_at(at: str, slot: str, now) -> tuple:
    """(час, минута) выхода слота с учётом дня недели"""
    hh, mm = (int(x) for x in at.split(":"))
    if now.weekday() >= 5:                    # суббота и воскресенье
        minutes = hh * 60 + mm + WEEKEND_LATER.get(slot, 0)
        hh, mm = divmod(min(minutes, 23 * 60 + 59), 60)
    return hh, mm


# Фраза дня в утреннем выпуске. Не лозунги про успех: читатель — человек,
# который в восемь утра открывает телефон перед рабочим днём, и бодрый
# плакат его раздражает. Здесь короткие мысли о работе, которые можно
# вспомнить к обеду.
MOTTOS = [
    "Дело двигается не вдохновением, а расписанием.",
    "Сегодня хватит одного шага — но сделанного.",
    "Занятость и работа — разные вещи. Выберите вторую.",
    "Не всё, что срочно, важно. Проверьте список.",
    "Сложное дело начинается с того, что вы открыли файл.",
    "Кто спрашивает, выглядит глупо минуту. Кто не спросил — дольше.",
    "У хорошего решения обычно скучный вид.",
    "Считайте не часы за столом, а сделанное.",
    "Ошибку дешевле признать сегодня, чем защищать месяц.",
    "Договорённость без даты — это пожелание.",
    "Если дело откладывается третий раз, оно либо чужое, либо ненужное.",
    "Прежде чем ускоряться, проверьте направление.",
    "Сделанное на восемьдесят процентов лучше задуманного на сто.",
    "Один разговор экономит неделю переписки.",
    "Планы не сбываются целиком — и всё-таки без них хуже.",
    "Умение остановиться стоит дороже умения терпеть.",
    "Никто не знает, что вы делаете, пока вы не скажете.",
    "Цена «потом» выясняется позже и всегда выше.",
    "Мелочь, которая раздражает каждый день, чинится один раз.",
    "Утро — единственное время, которое ещё никем не занято.",
    "Начатое вчера сегодня уже опыт.",
    "Лучше неудобная правда в отчёте, чем красивая цифра.",
]

# На выходных тон другой: человек не на работе, и подгонять его незачем.
MOTTOS_WEEKEND = [
    "Отдых — часть работы, а не награда за неё.",
    "Дела подождут ровно столько, сколько вы решите.",
    "Выходной, проведённый в телефоне, выходным не считается.",
    "Лучшие мысли приходят, когда их не требуют.",
    "Сегодня можно ничего не успеть. Это тоже план.",
    "Люди помнят, как вы себя вели, а не как вы были заняты.",
    "Отложенный разговор с близкими дороже отложенной задачи.",
    "Хорошая идея переживёт два выходных. Проверено.",
    "Тишина в чате в субботу — признак порядка, а не забвения.",
]

MOTTO_KEY = "digest_motto"       # «нет» — выключить фразу


def _motto(now) -> str:
    """Фраза на сегодня. Одна и та же весь день, разная в соседние дни."""
    bank = MOTTOS_WEEKEND if now.weekday() >= 5 else MOTTOS
    return bank[now.toordinal() % len(bank)]
...
async def _due_slot():
    ...
    # Порядок берём по времени, а не по тому, как слоты записаны: на
    # выходных сдвинутое утро меняет их местами, и «самый поздний из
    # наступивших» иначе выбирался бы неверно.
    day = sorted(((_slot_at(at, slot, now), slot, greeting)
                  for at, slot, greeting in SCHEDULE), key=lambda x: x[0])

    ready = None
    for (hh, mm), slot, greeting in day:
...
# Только путешествия. Помощь со звонком отвечает на вопрос, который
# возникает в чужой стране, — и больше нигде. Под новостями и генетикой
# она выглядела бы как реклама, пришитая ко всему подряд, а такое
# перестают замечать целиком.
CALL_SECTIONS = ("travel",)
...
async def publish_slot(bot: Bot, force: str = None) -> str:
    ...
    if slot == "morning":
        ...
        if not news:
            ...
            return "новостей нет"
        parts = [f"☀️ {greeting}"]
        if (await database.get_setting(MOTTO_KEY) or "").strip().lower() != "нет":
            # Курсивом и отдельной строкой: это не новость, и мешать её с
            # заголовками нельзя.
...
async def plan_command(message: Message):
    ...
    for day in range(1, days + 1):
        ...
        date = that_day.strftime("%d.%m")
        lines.append(f"\n<b>{date}</b>")
        for at, slot, _ in SCHEDULE:
            hh, mm = _slot_at(at, slot, that_day)
            at = f"{hh:02d}:{mm:02d}"
            if slot == "morning":
...
async def digest_command(message: Message, bot: Bot):
    ...
    if command == "motto":
        now = await _local_now()
        if len(parts) > 2:
            off = parts[2].strip().lower() in ("нет", "off", "выкл")
            await database.set_setting(MOTTO_KEY, "нет" if off else "")
            await message.answer("✅ Фраза дня убрана из утреннего выпуска."
                                 if off else
                                 f"✅ Фраза дня вернулась. Сегодня: _{_motto(now)}_"
                                 .replace("_", ""))
            return
        state = "выключена" if (await database.get_setting(MOTTO_KEY)) == "нет" else "включена"
        await message.answer(
            f"💬 <b>Фраза дня</b> — {state}\n\n"
            f"Сегодня: <i>{html.escape(_motto(now))}</i>\n\n"
            f"Запас: {len(MOTTOS)} на будни, {len(MOTTOS_WEEKEND)} на выходные. "
            f"Меняется раз в сутки.\n"
            f"Убрать: <code>/digest motto нет</code>")
        return
