...
async def init_db():
    ...
    async with pool.acquire() as conn:
        ...
        # Вторая ссылка — второй магазин. Бумага и файл это разный товар, и
        # читателю нужен выбор; одной ячейкой он превращался в «или-или».
        await conn.execute(
            f"ALTER TABLE {SCHEMA}.books ADD COLUMN IF NOT EXISTS buy_url2 TEXT")
        ...
        # Если таблица досталась от версии без разделения на календари —
        # CREATE TABLE IF NOT EXISTS колонку не дорисует.
        await conn.execute(
            f"ALTER TABLE {SCHEMA}.plan_events "
            "ADD COLUMN IF NOT EXISTS space_id INTEGER NOT NULL DEFAULT 0")
...
async def books_with_link():
    ...
    try:
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                f"SELECT id, text_content, "
                "COALESCE(NULLIF(buy_url, ''), buy_url2) AS buy_url "
                f"FROM {SCHEMA}.books "
                "WHERE COALESCE(buy_url, '') <> '' OR COALESCE(buy_url2, '') <> '' "
                "ORDER BY id")
...
async def get_book_links(title: str):
    """Все свои ссылки книги по первой строке — их может быть две"""
    try:
        pool = await get_pool()
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                f"SELECT buy_url, buy_url2 FROM {SCHEMA}.books "
                "WHERE text_content LIKE $1 LIMIT 1", f"{title[:60]}%")
        if not row:
            return []
        return [u for u in (row["buy_url"], row["buy_url2"]) if u]
    except Exception as e:
        logging.error(f"Ссылки книги недоступны: {e}")
        return []


async def book_links_by_id(book_id: int):
    """(первая ссылка, вторая) книги по номеру"""
    try:
        pool = await get_pool()
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                f"SELECT buy_url, buy_url2 FROM {SCHEMA}.books WHERE id = $1",
                book_id)
        return (row["buy_url"] or "", row["buy_url2"] or "") if row else ("", "")
    except Exception as e:
        logging.error(f"Ссылки книги недоступны: {e}")
        return "", ""


async def set_book_link_slot(book_id: int, url: str, second: bool) -> bool:
    """Записать ссылку в первую или вторую ячейку"""
    column = "buy_url2" if second else "buy_url"
    try:
        pool = await get_pool()
        async with pool.acquire() as conn:
            done = await conn.execute(
                f"UPDATE {SCHEMA}.books SET {column} = $1 WHERE id = $2",
                url or None, book_id)
        return done.endswith("1")
    except Exception as e:
        logging.error(f"Ссылка книги не сохранилась: {e}")
        return False


async def books_links_all():
    """(id, название, первая ссылка, вторая) — для прохода по всем книгам"""
    try:
        pool = await get_pool()
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                f"SELECT id, text_content, buy_url, buy_url2 FROM {SCHEMA}.books "
                "ORDER BY id")
        return [(r["id"], (r["text_content"] or "").split("\n")[0][:70],
                 r["buy_url"] or "", r["buy_url2"] or "") for r in rows]
    except Exception as e:
        ...
        async with pool.acquire() as conn:
            if category:
                rows = await conn.fetch(
                    ...
                    "WHERE category = $1 AND COALESCE(buy_url, '') = '' "
                    "AND COALESCE(buy_url2, '') = '' ORDER BY id", category)
            else:
                rows = await conn.fetch(
                    f"SELECT id, text_content FROM {SCHEMA}.books "
                    "WHERE COALESCE(buy_url, '') = '' "
                    "AND COALESCE(buy_url2, '') = '' ORDER BY id")
        return [(r["id"], (r["text_content"] or "").split("\n")[0][:70]) for r in rows]
    except Exception as e:
...
async def books_link_stats() -> dict:
    try:
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                f"SELECT COUNT(*) AS total, "
                f"COUNT(*) FILTER (WHERE COALESCE(buy_url, '') <> '' "
                f"                    OR COALESCE(buy_url2, '') <> '') AS done "
                f"FROM {SCHEMA}.books")
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                space_id = await conn.fetchval(
                    ...
                    title, owner_id, owner_name, invite_code, paid_until)
                # owner_id = 0 — календарь ещё не передан: его заберёт первый,
                # кто откроет ссылку. Записывать «участника с номером ноль» нельзя.
                if owner_id:
                    await conn.execute(
...
async def plan_space_claim(space_id: int, user_id: int, user_name: str) -> bool:
    """Первый, кто открыл ссылку на непереданный календарь, становится админом.

    Условие owner_id = 0 стоит в самом UPDATE: если ссылку одновременно откроют
    двое, календарь достанется одному, а не последнему записавшему.
    """
    try:
        pool = await get_pool()
        async with pool.acquire() as conn:
            async with conn.transaction():
                    ...
                    f"UPDATE {SCHEMA}.plan_spaces SET owner_id = $2, owner_name = $3 "
                    "WHERE id = $1 AND owner_id = 0", space_id, user_id, user_name)
                if not done.endswith("1"):
                    return False
                await conn.execute(
                    f"INSERT INTO {SCHEMA}.plan_members (space_id, user_id, user_name, role) "
                    "VALUES ($1, $2, $3, 'owner') ON CONFLICT (space_id, user_id) "
                    "DO UPDATE SET role = 'owner', user_name = EXCLUDED.user_name",
                    space_id, user_id, user_name)
        return True
    except Exception as e:
        logging.error(f"Не удалось передать календарь {space_id}: {e}")
        return False


async def plan_space_recode(space_id: int, code: str) -> bool:
    """Новый код приглашения: старая ссылка после этого никого не пускает"""
    try:
        pool = await get_pool()
        async with pool.acquire() as conn:
            done = await conn.execute(
                f"UPDATE {SCHEMA}.plan_spaces SET invite_code = $2 WHERE id = $1",
                space_id, code)
        return done.endswith("1")
    except Exception as e:
        logging.error(f"Не удалось сменить код приглашения {space_id}: {e}")
        return False
